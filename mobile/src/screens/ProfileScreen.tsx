...
import type { ClimbLog, UserBadge } from '../types';
...
function computeStreak(isoDates: string[]): number {
  ...
  // Grace: if nothing logged yet today, start counting from yesterday instead
  // of immediately reporting a broken streak.
      ...
      >
        ...
        <View style={s.header}>
          ...
          <Text style={[s.displayName, isDark && { color: '#F9FAFB' }]}>
            {isAnonymous ? 'Guest Climber' : (user?.user_metadata?.full_name ?? user?.user_metadata?.display_name ?? 'Climber')}
          </Text>
        ...
        <View style={s.section}>
          ...
          {climbHistory.length > 0 ? (
            climbHistory.slice(0, 20).map((climb, i) => {
              ...
              return (
                <View key={i} style={[s.climbRow, isDark && { backgroundColor: '#1F2937' }]}>
                  ...
                  <View style={s.climbContent}>
                    ...
                    <Text style={s.climbDate}>{formatRelativeTime(climb.climbedAt)}</Text>
                  </View>
