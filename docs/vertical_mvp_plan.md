...
**Current State (as of 2026-07-12):** Phases 1, 1.5, 2a (auth, badge/verification system, building photos, notifications), 2b (social feed, leaderboard, public profiles, kudos/comments), and what was informally tracked as "2c" through "2e" are shipped. Beyond the original MVP scope, the app now has: a Groups tab (challenges — official and user-created, public or peers-only, with per-challenge leaderboards and custom medal badges; official app-run clubs with weekly organizer-only channels; events with real venue photos); a shared, community-verified amenity-reporting system (Supabase-backed, 3-confirmation verification, comments, anti-spam caps) covering both user-submitted reports and the originally-static water-cooler dataset; a Home tab (weekly/monthly goals, a progressive-overload projection for next month, a 31-day calendar with local reminders) that replaced Profile on the bottom tab bar — Profile itself (badges, climb history, settings) is now reached by tapping your own avatar in Social or Groups and opens as an overlay; social posts support up to 6 photos per post; onboarding's path/motivation/fitness answers now jointly compute the starting weekly goal and a suggested climb cadence, instead of fitness level alone; and `ClimbTrackerModal`'s barometer tracking now fuses in the accelerometer to distinguish real stepping (ascent/descent) from an elevator/escalator ride, which the barometer alone couldn't tell apart. Photos are now compressed on upload (`expo-image-manipulator`, ≤1200px/JPEG-0.8 — a native module, so it engages after the next dev-client rebuild and falls back to the original until then); the Groups tab bar is a native-driven Strava-style collapsing header; every badge renders one unified medal (hand-drawn emblem for challenge archetypes, the badge's own icon otherwise) consistently in the shelf, tap-in detail, and challenge cards; the events list was expanded to real international vertical races with openly-licensed landscape venue/skyline photos; and there's an in-app Help & Feedback screen (FAQ + a report form writing to a `feedback` table, plus a community Telegram link). Also: a browser-preview build (`expo start --web`) for the non-native-dependent screens. Auth is anonymous-first (`signInAnonymously`, upgradeable), not "none" as originally locked below — see the Locked Tech Stack table for the MVP-era decision this superseded. Not yet built: avatar-frame challenge rewards, in-club threaded replies, live/editable event listings (currently hardcoded + curated — a `curated_events` Supabase table would make them updatable post-launch without a rebuild), push-notification climb reminders (local-only for now), device-calibrated sensor-fusion thresholds (current constants are reasonable defaults, not tuned against real hardware), and full dark-mode parity on a few older modals (`PublicProfileModal`, `ClimbTrackerModal`, `BuildingDetailSheet` are still light-only).
